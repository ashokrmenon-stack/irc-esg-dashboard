...
export default async function Page() {
  ...
  const trend = years.map(y => ({ year: y, ["Water withdrawn (m³)"]: irc.find(r => r.year === y)?.total_withdrawn_areas_m3 ?? null }));

  const byEntity = rows
    .filter(r => r.year === latest && r.location_code !== "IRC" && r.total_withdrawn_areas_m3 != null)
    .sort((a, b) => (b.total_withdrawn_areas_m3 ?? 0) - (a.total_withdrawn_areas_m3 ?? 0));
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-white border border-gray-200 rounded-lg p-5">
        ...
        <TrendChart
          data={trend}
          series={[{ key: "Water withdrawn (m³)", label: "Water withdrawn (m³)", color: "#0d9488" }]}
          yLabel="m³"
        />
      </section>
      <section className="bg-white border border-gray-200 rounded-lg p-5">
        <h2 className="font-medium mb-4">By entity ({latest})</h2>
        <BarYearByEntity data={byEntity} dataKey="total_withdrawn_areas_m3" label="Water withdrawn (m³)" />
      </section>
    </div>
  );
}
